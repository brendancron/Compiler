...
impl Pipeline<String, Vec<BlueprintStmt>, CompilerError> {
        ...
        W: Write + 'static,
    {
        self.then(move |parsed| {
            ...
            let processed =
                metaprocessor::process(&parsed, &mut mp_ctx).map_err(|e| format!("{:?}", e))?;
...
impl Pipeline<String, (Vec<ExpandedStmt>, DeclRegistryRef), CompilerError> {
    pub fn with_interpreter<W>(self, mut out: W) -> Pipeline<String, (), CompilerError>
    where
        W: Write + 'static,
    {
        self.then(move |(expanded, decl_reg)| {
            let env = Env::new();
            interpreter::eval(&expanded, env, decl_reg, &mut None, &mut out)
                .map_err(|e| format!("{:?}", e))?;
            Ok(())
        })
    }
...
impl Pipeline<String, (Vec<ExpandedStmt>, DeclRegistryRef), CompilerError> {
    ...
    pub fn dump_expanded_code<P: AsRef<std::path::Path>>(self, out_dir: P) -> Self {
        ...
        self.tap(move |(expanded, _)| {
            fs::create_dir_all(&out_dir).map_err(|e| e.to_string())?;
            let mut f =
                File::create(out_dir.join("expanded_code.cx")).map_err(|e| e.to_string())?;
