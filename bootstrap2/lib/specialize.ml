...
let rec type_directed_expr (e : Ast.typed_expr) =
  ...
  | `Collection_lit items -> Types.has_generic e.Ast.ann || List.exists type_directed_expr items
...
let copy_for state name (at : Types.ty) =
  ...
  | None ->
    let copy = Printf.sprintf "%s__%d" name (Hashtbl.length state.copies) in
...
let rec rewrite state (e : Ast.typed_expr) : Ast.typed_expr =
  let it : Ast.typed_expr_kind =
    ...
    | #Ast.method_call as m ->
      (Ast.map_method_call (rewrite state) m :> Ast.typed_expr_kind)
